...
In the previous chapter, we talked about how you'd set up a real URL for your data science server with a real URL and `https` for security. If you've done all that right, that will get internet traffic to your server's front door. In this chapter, we'll talk about how to get traffic from the front door to the right place.

You might not have known it, but when you set up the server in the first chapter, you also actually created a private network for the server to live in called a *virtual private cloud (VPC)*.

As your server configuration gets more complicated, you're going to put more different servers inside the virtual private cloud. So instead of thinking of your traffic as coming to the front door of your little server/cottage, I think it's a more helpful analogy to think of coming to the guard box at the front gate of a gated community.
