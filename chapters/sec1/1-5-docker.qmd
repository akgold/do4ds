...
Docker is by far the most popular open-source containerization platform. So much so that for most purposes container is a synonym for Docker container.[^1-5-docker-1] In this chapter, containers will exclusively refer to Docker containers.

[^1-5-docker-1]: There are some other types of containers that are commonly used for specialized use cases, like high-performance computing.
...
But this is also completely non-reproducible. Whenever you rebuild your container, you'll install the newest versions of Shiny and Dplyr afresh, potentially ruining the reproducibility of your code. For that reason, the best move is to still use R- and Python-specific libraries for capturing package state -- like `renv` and `rig` in R and `virtualenv` , `conda` , and `pyenv` in Python -- rather than relying on Docker for that job. There's more on those topics in the [chapter on environments](#repro).
...
Port forwarding is always specified as `<host port>:<container port>`. Try playing around with changing the values to make the API available on a different port, perhaps `9876`. For a more in-depth treatment of ports, see [Chapter @sec-networks-1].
...
1.  What does using a Docker container for a data science project make easier? What does it make harder?
2.  Draw a mental map of the relationship between the following: Dockerfile, Docker Image, Docker Registry, Docker Container
3.  When would you want to use each of the following flags for `docker run`? When wouldn't you?
    -    `-p`, `--name`, `-d`, `--rm`, `-v`
...
-   Hint 3: If you're running a laptop with Apple Silicon (M1 or M2), you may need to try a different container. `amoselb/rstudio-m1` worked for me.[^1-5-docker-3]

[^1-5-docker-3]: This should eventually be resolved, keep an eye on [this issue](https://github.com/rstudio/rstudio/issues/8809) in the RStudio repo.
