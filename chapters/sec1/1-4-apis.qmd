...
| End?         |              | Engineering  |                       |
+==============+==============+==============+=======================+
| Front End    | Pre          | Javascript   | Shiny/Dash/Streamlit  |
|              | sentation    |              |                       |
|              |              |              | Quarto/R              |
|              |              |              | Markdown/Jupyter      |
+--------------+--------------+--------------+-----------------------+
| Back End     | Ap plication | API          | Functions *or* API    |
+--------------+--------------+--------------+-----------------------+
...
|                 | received, but there was an error processing it. |
+-----------------+-------------------------------------------------+
...
## Lab 4: Turn your Penguins model into an API

In this lab, we're going to create an API from our Penguin prediction
model and get some predictions out.

The `{vetiver}` package makes it extremely easy to generate a Python
language `fastAPI` from a `scikit-learn` model. It's also worth noting
that `{vetiver}` also supports turning R language models into
`{plumber}` APIs.
...
You can add this code to the end of your `model.qmd`.

``` {.python eval="false" include="../../labs/lab-4/model-vetiver.qmd" filename="model.qmd" start-line=44}
```

Once you've done this, you can run `app.run(port = 8080)` in your Python session. This will run your model API on port 8080. More on what that means in [Chapter @sec-basic-networks]. 

::: callout-note
Take a look at line 52. In this line, we're writing our model to a temporary board. That's because we're just running this API locally. Later on, when we move the API to a permanent home in [Chapter @sec-linux-admin], we'll also need to put the model somewhere so it can be loaded by the API.
:::

Once it's running, you can access it by going to `http://127.0.0.1:8080` in your browser. You can play around with the front end there, including trying the provided examples.
...
```{r}
#| eval: false
r <- httr2::request("http://127.0.0.1:8080/ping") |>
  httr2::req_perform()
...
It's worth noting that the `{vetiver}` package also includes the ability to auto-query a vetiver API without having to manually perform the request in R or Python. That capability is documented in the [vetiver package documentation](https://vetiver.rstudio.com/get-started/deploy.html#predict-from-your-model-endpoint).
