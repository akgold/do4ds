...
| End?         |              | Engineering  |                    |
+==============+==============+==============+====================+
| Front End    | Pre          | Javascript   | Sh                 |
|              | sentation    |              | iny/Dash/Streamlit |
|              |              |              |                    |
|              |              |              | Quarto/R           |
|              |              |              | Markdown/Jupyter   |
+--------------+--------------+--------------+--------------------+
...
Once you've done this, you can run `app.run(port = 8080)` in your Python
session. This will run your model API on port 8080.

More on what that means in [Chapter @sec-basic-networks].

::: callout-note
Take a look at line 52. In this line, we're writing our model to a
temporary board. That's because we're just running this API locally.
Later on, when we move the API to a permanent home in [Chapter
@sec-linux-admin], we'll also need to put the model somewhere so it can
be loaded by the API.
:::

Once it's running, you can access it by going to `http://127.0.0.1:8080`
in your browser. You can play around with the front end there, including
trying the provided examples.
...
It's worth noting that the `{vetiver}` package also includes the ability
to auto-query a vetiver API without having to manually perform the
request in R or Python. That capability is documented in the [vetiver
package
...
``` {.r eval="false" include="../../labs/lab-4/app.R" filename="app.R"}
```

And in Python, it looks like this

``` {.python eval="false" include="../../labs/lab-4/app.py" filename="app.py"}
```
