...
| project directory.   |                      |                |
+----------------------+----------------------+----------------+
| Make sure you've got | `ins t a             | Included w/    |
| `{renv}/{venv}`.     | ll.packages("renv")` | Python 3.5+    |
+----------------------+----------------------+----------------+
...
|                      |                      | env for <dir>` |
+----------------------+----------------------+----------------+
| Activate project     | `renv::activate()`   | `so u          |
| library.             |                      | rce < d i r >  |
|                      | Happens              | /bin/activate` |
|                      | automatically if     |                |
|                      | using projects.      |                |
+----------------------+----------------------+----------------+
| Install packages as  | `inst a l            | `pyt           |
| normal.              | l.packages("<pkg>")` |  hon -m p i p  |
|                      |                      | install <pkg>` |
+----------------------+----------------------+----------------+
| Snapshot package     | `renv::snapshot()`   | `p ip f        |
| state.               |                      | reeze > r e q  |
|                      |                      | uirements.txt` |
+----------------------+----------------------+----------------+
...
+----------------+-----------------------+----------------+
| Move into      | `s e                  | `c d           |
| project        | twd("<project-dir>")` | <project-dir>` |
| directory.     |                       |                |
|                | Or just open R        |                |
|                | project in RStudio.   |                |
+----------------+-----------------------+----------------+
...
|                |                       | env for <dir>` |
+----------------+-----------------------+----------------+
| Enter project  | Happens               | `so            |
| environment.   | automatically.        | urce < d i r > |
|                |                       | /bin/activate` |
+----------------+-----------------------+----------------+
| Restore        | May happen            | `p ip ins      |
| packages.      | automatically or      | tall - r r e q |
|                | `renv::restore()`     | uirements.txt` |
+----------------+-----------------------+----------------+
