...
| project directory.   |                      |                      |
+----------------------+----------------------+----------------------+
| Make sure you've got | `ins t a l           | Included w/ Python   |
| `{renv}/{venv}`.     | l .packages("renv")` | 3.5+                 |
+----------------------+----------------------+----------------------+
| Create a standalone  | `renv::init()`       | `pyt                 |
| library.             |                      | h o n -m venv <dir>` |
|                      |                      |                      |
|                      |                      | Recommend:           |
|                      |                      | `us                  |
|                      |                      | e . v env for <dir>` |
+----------------------+----------------------+----------------------+
| Activate project     | `renv::activate()`   | `so u rce < d        |
| library.             |                      | i r > /bin/activate` |
|                      | Happens              |                      |
|                      | automatically if     |                      |
|                      | using projects.      |                      |
+----------------------+----------------------+----------------------+
| Install packages as  | `inst a l l          | `pyt  hon -m         |
| normal.              | . packages("<pkg>")` | p i p install <pkg>` |
+----------------------+----------------------+----------------------+
| Snapshot package     | `renv::snapshot()`   | `p ip f reeze >      |
| state.               |                      | r e q uirements.txt` |
+----------------------+----------------------+----------------------+
...
| Download project.    |                      |                      |
+----------------------+----------------------+----------------------+
| Move into project    | `s e t w             | `c d <project-dir>`  |
...
| environment.         |                      | h o n -m venv <dir>` |
|                      |                      |                      |
|                      |                      | Recommend:           |
|                      |                      | `u s                 |
|                      |                      | e . v env for <dir>` |
+----------------------+----------------------+----------------------+
| Enter project        | Happens              | `so u rce < d        |
| environment.         | automatically.       | i r > /bin/activate` |
+----------------------+----------------------+----------------------+
| Restore packages.    | May happen           | `p ip ins t all - r  |
|                      | automatically or     | r e q uirements.txt` |
|                      | `renv::restore()`    |                      |
+----------------------+----------------------+----------------------+
...
``` {.markdown filename="eda.qmd" include="../../_labs/eda/eda_basic.qmd"}
```
...
``` {.yml filename="model.qmd" include="../../_labs/model/model_basic.qmd"}
```
