...
If you're in the data science world, especially the world of "production data science", you've probably heard of Docker and Kubernetes. But you might not really be sure what they are or whether you need them.

This chapter is designed to clarify what these tools are and whether you need it as a data scientist. We'll play around with Docker and Kubernetes a little, but Docker and Kubernetes are some of the hottest things out there right now, so there are a huge variety of resources for learning. I'd recommend you'd check them out if you're interested.

The last few years have seen the rise of containerization as a strategy for managing applications in enterprise software deployments. The main benefit of containerization is that it allows you to create a container that includes both an application you want to run, as well as all its dependencies.
...
\[TODO: Find a good example to have them run\]
...
In the last 5-10 years, containers have exploded in popularity. The first containerization platform to make it big was Docker, and it's still the biggest. Docker is an open-source containerization platform. Depending on who you are, you might like Docker for different reasons -- developers and data scientists tend to love Docker because it allows them to ship all their app dependencies with their app. IT/Admins tend to love Docker because apps show up pre-packaged, because Docker makes it really easy to scale up apps in a "microservices architecture", and because you can tightly control the resources available to a Docker container, making it more secure and easier to ensure system stability.
...
## Using Containers as a Data Scientist
