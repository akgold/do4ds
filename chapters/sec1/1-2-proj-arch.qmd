...
you're creating a report, \index{API}, book, or paper.
...
2.  *An app.* Data science apps are created in frameworks like \index{Shiny} (R
...
4.  *An \index{API}* *(application programming interface)*. An \index{API} is for
...
[^1-2-proj-arch-1]: Though I'll argue in [Chapter @sec-log-monitor] that
    ...
    Markdown, or \index{Jupyter Notebook}.
...
thinking about a job or \index{API}. You should create a job if it runs in a
batched way, i.e. you write a data file or results into a database. If
you want results to be queried in real-time, it's an \index{API}.
...
![](images/presentation-layer.png){fig-alt="A flow chart of choosing an App, Report, \index{API}, or Job for the presentation layer as described in this section."
}
...
you're writing a \index{Shiny} app. You want to use the presentation layer to do
...
Microsoft365 sites, and \index{Posit} Connect.

### Google Sheets

If you're still early in your project lifecycle, a \index{Google Sheet} can be a
...
The primary weakness of a \index{Google Sheet} -- that it's editable by someone
...
## Create an \index{API} if you need it

In the case of a general-purpose three-layer app, it is almost always
the case that the middle tier will be an \index{API}. Separating processing
logic into functions is often sufficient in a data science app. But
separating it into an \index{API} is often helpful if you've got a long-running
bit of business logic, like training an ML model.

::: callout-note
You may have heard the term REST \index{API} or REST-ful.

REST is a set of architectural standards for how to build an \index{API}. An \index{API}
that conforms to those standards is called REST-ful or a REST \index{API}.

If you're using standard methods for constructing an \index{API} like R's
`{plumber}` package or `{FastAPI}` in Python, they will be REST-ful --
or at least close enough for standard usage.
:::

You can think of an \index{API} as a "function as a service". That is, an \index{API} is
...
separate \index{API}, it allows you to think about scaling out the presentation
layer separate from the business layer.

Luckily, if you've written functions for your app, turning them into an
\index{API} is trivial since packages like `{fastAPI}` and `{plumber}` let you
turn a function into an \index{API} by adding some specially-formatted comments.
...
![](images/data-flow-chart.png){fig-alt="A data flow chart showing how the palmer penguins data flows into the model creation \index{API}, which creates the model and the model creation report. The model serving \index{API} uses the model and feeds the model explorer \index{API}."
}
...
### Step 2: Create an \index{API} for model predictions

I'll serve the model from an \index{API} to allow for real-time predictions.
...
can then access run your model \index{API} by navigating to
