...
# TODO: Separate API/HTTP into separate chapter.

# HTTP and other Application Layer Protocols {#sec-http}

TODO: add more on resources, query parameters, and anchors

### `http` is *the* protocol of the web

Hyptertext transfer protocol (`http`) is the protocol you'll encounter
most commonly, as it is the way all web traffic is conducted. `http` is
a request-response protocol -- one machine initiates a request and
another one answers.

Let's take a simple example -- going to visit a website in your browser.
...
Just as a physical letter has a prescribed format that starts with the
date and a greeting and closes with a signature. Similarly, `http`
requests and responses include metadata in the form of *headers.*

Headers specify information about the request or response. These often
include the type of machine that is sending the request, authentication
credentials or tokens, cookies, and more.

Then there's the *body* of your request. Like the body of a letter, this
is the actual content of the message. The `http` protocol specifies how
that body gets packaged up, but the contents are up to the service
that's receiving the message.
...
It's worth noting that `GET` requests generally don't include a body.

Instead, specifics are specified through *query parameters*, the part of
the URL that shows up after the `?`, like
...
computer and the server. You can see the response codes right next to
each request, in chrome, codes other than 200s show up in other colors
...
1.  For each of the following, are they a concern of TCP/IP or an
    application layer protocol: routing packets, defining a server API,
    defining valid IP addresses, specifying maximum request size,
    defining server error codes.
2.  Can a server ever initiate an `http` request?
