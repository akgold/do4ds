...
## Lab: Host a Website on GitHub

Being able to work well with git is a fundamental skill for adhering to
DevOps best practices. For most organizations, that means using GitHub.

In this lab, we're going to put up a website using GitHub Pages and set
up GitHub actions to update the website whenever we make changes.

We're going to use [Quarto](https://quarto.org) for our website. Quarto
is a open source scientific and technical publishing system that makes
it easy to render R and Python code into beautiful documents, website,
reports, and presentations.

Here's what you should do:

1.  Create an empty remote git repo and clone it locally.
2.  Create a quarto doc in the repo.
3.  Follow the [instructions on the Quarto
    website](https://quarto.org/docs/publishing/github-pages.html#github-action)
    to serve it on GitHub pages and steps to run from GitHub Actions.
4.  Configure GitHub pages to serve the site (off of `gh-pages`) branch.

If you follow the instructions on the quarto website, you'll end up with
a workflow called `publish.yml`. Let's pull apart what's happening in
this `yml` file -- it should be reasonably human-readable.

``` {.yaml include="../../labs/lab-1/publish.yml" filename=".github/workflows/publish.yml"}
```

The top section just defines what triggers this workflow. In this case,
we've configured the workflow only to trigger on a push to the `main`
branch. Note that a merge into main counts.

Below that we define a name for the workflow and what actually happens.

There are a few things that are defined here.

What kind of host does the job run on?

What does it do? You can write custom GitHub Actions steps, but these
days most things you want to do are better accomplished by taking
existing workflows and providing arguments to them. You can see that
this uses 3 existing workflows to check out a repository, set up Quarto,
and render and publish a Quarto doc. There are no arguments to the first
two steps, but the third step uses two arguments. `with` defines
...
Once you've finished this lab, you'll have a website up and running! In
the next lab, we'll add some R and Python content to the website.
