...
know when issues arise and can figure out why.
...
can also help demonstrate its value. Being able to show who is using
...
integrate with tooling your organization already has for aggregation.
...
to know whether their software uses too much RAM or CPU, is fast enough,
or has crashed.

For a general-purpose software engineer, an uncaught exception that
makes the software crash is about as bad as it gets.

For a data scientist, something even scarier is an issue that doesn't
result in code failure but yields incorrect answers. Data joins usually
...
The first step to making your app or \index{API} observable is to add logging.
...
emit logs in useful formats, and provide visibility into the severity of
issues.
...
Plain text logging is an excellent choice if the only use of the logs is
...
If you're running in a \index{Docker} \index{Container}, you need to direct the logs
outside the \index{Container}. This is usually accomplished by sending normal
...
inside a \index{Docker} \index{Container} is ephemeral. This is bad if you're writing a
log that might contain clues for why a \index{Docker} \index{Container} was unexpectedly
killed.
:::
...
about, like data quality or \index{API} response times. There is an official
Prometheus client in Python and the `{openmetrics}` package in R makes
registering metrics from a Plumber \index{API} or \index{Shiny} app easy.
...
3.  At what level would you log each of the following events:
    1.  Someone clicks on a particular tab in your \index{Shiny} app.

    2.  Someone puts an invalid entry into a text entry box.

    3.  An HTTP call your app makes to an external \index{API} fails.
