...
But you can live in a world where you sleep soundly, assured that you'll
know when issues arise and that you'll be able to figure out why.
...
can also help you demonstrate its value. Being able to show who is using
your work and how can help decision makers understand that your team
matters.

There are two halves to observability -- emitting the logs and metrics
that reveal what's going on inside your project and aggregating and
...
integrate with tooling your organization alread has for aggregation.
...
fast enough, or whether its crashed.

For a general purpose software engineer, an uncaught exception that
makes the software crash is about as bad as it gets.

For a data scientist, there's something even scarier, an issue that
doesn't result in code failure but yields incorrect answers. Data joins
usually complete even if the merge quality is terrible. Model APIs will
return a prediction even if the prediction is very, very bad.

Checking correctness of the numbers and figures you produce is hard
because data scientists are (basically by definition) doing something
novel. The solution is to put process metrics in place to hopefully
reveal a problem before it surfaces in your results.

One important tool is correctly architecting your project. Jobs are
generally much easier to check for correctness than presentation layers.
By moving as much processing as possible out of the presentation layer
and into the data and processing layers, you can make it easier to
observe.
...
unique ids), I know how many rows should be in the data set after a
join. Figuring out how many rows to expect can take a minute, but
checking that the size of the joined data matches my expectations has
avoided many gnarly issues.
...
way to spot novel values to make sure they're recoded correctly.
...
these other than that you need to use one if you've got a model that's
producing results you hope to rely on.

## Observing Operations

As a data scientist, you can't only pay attention to correctness. If you
want people to find your work valuable, you still need to pay attention
to the operational qualities of your code like the speed and
responsiveness, system resources it's consuming, the number of users,
and user interactions just before an error occurs.

The first step to making your app or API observable is to add logging.
You may be accustomed to just adding `print` statements throughout your
code. Honestly, this is far better than nothing. But purpose-built
tooling for logging allows you to apply consistent formats within logs,
emit logs in useful formats, and provide visibility into severity of
issues.

There are great logging packages in both Python and R. Python's logging
package is standard and included. There is not a standard logging
package in R, but I recommend `{log4r}`.
...
your code does something, you'll use the log session to write *log
statements*. When the log statement runs, it creates a *log entry*.
...
-   The *log metadata* is data that the logging library automatically
...
1.  *Debug*: detail on what the code was doing. Debug statements are
    designed to be useful to make sense to someone who really knows the
    code. For example, you might include which function ran and with
    what arguments in a debug log.
...
The default for most logging is to emit logs in *plain text*. For
example, a plain text log of an app starting might put this on your
console
...
Plain text logging is a great choice if the only use of the logs is for
humans to read them. You might prefer *structured logs* if you're
...
By default, logs go to the console. This is a great choice in
development, because it makes it easy to see the logs as you go. If you
want to choose a different place for them, you can configure it with a
...
logs after a certain retention period via *log rotation.*

A common log rotation pattern is to have each individual log file last
for 24 hours and then be retained for 30 days before it is deleted. The
Python `{logging}` library does log rotation itself. `{log4r}` does not,
but there is a Linux library called `logrotate` that you can use in
...
want to move logs off of the server and into a centralized monitoring
...
If you're running in a Docker Container, you need to direct the logs
outside the Container. This is usually accomplished by sending normal
operating logs to *stdout* (usually pronounced standard out) and
failures to *stderr* (standard error).

::: callout-note
As you'll learn more about in [Chapter @sec-docker], anything that lives
inside a Docker Container is ephemeral. This is obviously bad if you're
writing a log that might contain clues for why a Docker Container was
unexpectedly killed.
:::
...
metrics data, query that data, and alert based on it. Grafana is an open
source dashboarding tool that sits on top of Prometheus to do
visualization of metrics. They are usually used together to do
...
just set up their service and point your app to it. Many organizations
also use DataDog, which is a SaaS service for log aggregation and
monitoring.
...
servers; however, it's not hard to use them to monitor things you might
care about like data quality or API response times.

If you want to register metrics from your API or app with Prometheus,
there is an official Prometheus client in Python and the `{openmetrics}`
package in R makes it easy to register metrics from a Plumber API or
Shiny app.

There's a great Get Started with Grafana and Prometheus doc on the
Grafana Labs website if you want to actually try it out.
...
You can feel free to log whatever you think is helpful. For example, it
would probably be useful to get the actual error contents if an HTTP
error comes back.
