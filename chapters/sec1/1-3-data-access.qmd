...
Usually, that will be in a database or a data \index{API}. This chapter is about
...
implement the \index{API}, which is why the example above calls the `connect()`
method of the `{psychopg2}` package.

In R, the \index{API} is split into two parts. The `{DBI}` package (short for
...
::: callout-note
There are Python packages that don't implement the connections \index{API} and
...
Some data sources come in the form of an \index{API}.

It's common to have Python or R packages that wrap APIs, so you write
Python or R code without thinking about the \index{API} underneath. Using these
patterns often looks similar to databases -- you create and use a
connection object that stores the connection details. If your \index{API} has a
package like this, you should use it.

If you're consuming a private \index{API} at your organization, a helper package
probably doesn't exist, or you may have to write it yourself.

::: callout-note
There's increasingly good tooling to auto-generate packages based on \index{API}
documentation, so you may never have to write an \index{API} wrapper package by
hand. It's still helpful to understand how it works.
:::

If you have to call an \index{API} directly, you can use the `{requests}`
...
special about calling an \index{API} from inside Python or R versus using the
command line and you can go back and forth as you please. It is
sometimes helpful to try to replicate Python or R \index{API} calls without the
language wrapper for debugging reasons.

### What's in an \index{API}?

APIs are the standard way for two computer systems to communicate. \index{API}
...
`http` operates on a request-response model. So when you use an \index{API}, you
send a request to the \index{API} and it sends a response back.
...
The best way to learn about a new \index{API} is to read the documentation,
which will include many details about usage. Let's go through some of
the most salient ones.

### \index{API} Endpoints and Paths

Each request to an \index{API} is directed to a specific *endpoint*. An \index{API} can
...
`{vetiver}` to create an \index{API} for serving the penguin mass model, you
found your \index{API} at `http://localhost:8080`. By default, you went to the
*root* path at `/` and found the \index{API} documentation there.
...
Looking at the penguin mass \index{API}, you'll see that `/ping` is a `GET`
endpoint and `/predict` is a `POST`. This isn't a coincidence. I'd
approximate that 95% of the \index{API} endpoints you'll use as a data scientist
...
[^1-3-data-access-2]: You may see `POST` for things that look like
    `GET`s. For example, fetching a model prediction from an \index{API} feels
    like a `GET` to me. The reason is the HTTP spec only recently
    allowed `GET` endpoints to use request bodies and it's still
    discouraged. So if the \index{API} wants to use a body in the request, it's
    likely to be a `POST` even if it's more of a `GET` activity.

### (Auth) Headers

Most APIs require authentication. The most common forms of
authentication are a username and password combination, an \index{API} key, or
an OAuth token.

\index{API} keys and OAuth tokens are often associated with particular *scopes*.
Scopes are permissions to do particular things. For example, an \index{API} key
...
*header* to your \index{API} call. Your \index{API} documentation will tell you how to
provide your username and password, \index{API} key, or token to the \index{API} in a
...
with your request and the \index{API} couldn't understand what you were asking.
...
`{jsonlite}`. Depending on the \index{API}, you may have the option to request
something other than JSON as the return. I rarely need anything other
than JSON.

### Common \index{API} patterns

Here are a couple of common \index{API} patterns that are good to know about:

-   **Pagination** -- many data-feed APIs implement pagination. A
    paginated \index{API} returns only a certain number of results to keep data
    sizes modest. Check out the \index{API} documentation to learn how to get
    all your pages.

-   **Job APIs** -- HTTP is designed for relatively quick
    request-response cycles. If your \index{API} kicks off a long-running job,
...
password or \index{API} key should never appear in your code.**
...
::: callout-note
This section assumes you can use a username and password or an \index{API} key
...
For example, GitHub Actions and \index{Posit} Connect both allow you to set
...
2.  Draw a mental map for using an \index{API} from R or Python. You should (at
...
## Lab: Use a database and an \index{API}
...
### Step 2: Call the model \index{API} from code

Before you start, ensure the \index{API} is running on your machine from the
last lab.
...
Here's what it looks like to call the \index{API} in Python
...
`{vetiver}` \index{API}. I'm not using it here to expose the details of calling
an \index{API}.
:::

Let's take this \index{API}-calling code and build the presentation layer around
it.
...
server <- function(input, output) {
  ...
  # Fetch prediction from \index{API}
  pred <- eventReactive(
    input$predict,
