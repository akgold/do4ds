...
Most often, bodies are in JSON and you'll decode them with `{json}` in Python or
`{jsonlite}` in R. Depending on the \index{API}API, you may have the option
to request something other than JSON as the return. I rarely need
...
For example, \index{GitHub Actions}GitHub Actions and \index{Posit}Posit Connect both
allow you to set secrets that aren't visible to the users but are
accessible to the code at runtime in an
...
::: callout-tip
If you're using my scripts, or just want to look at an example, you can find my scripts in the
GitHub repo for this book
([akgold/do4ds](https://github.com/akgold/do4ds)) in the [`_labs/lab3`](https://github.com/akgold/do4ds/tree/main/_labs/lab3)
directory. 
:::

In the R EDA script, we will replace our data loading with connecting to the database.
Leaving out all the parts that don't change, it looks
like:[^1-3-data-access-3]

[^1-3-data-access-3]: You may need to install and load the `{dbplyr}` package for this to work.
...
For the Python modeling script, we're just going to load the entire dataset into memory for
...
I'm going to use the \index{Shiny}`{shiny}` package to create an interactive web app. There are versions of `{shiny}` for both R and Python. 

If you want to take a detour into learning Shiny, I highly recommend the [Mastering
Shiny](https://mastering-shiny.org/) book. Otherwise feel free to just use my examples from the GitHub repo.

::: callout-tip
If you shut things down after the last lab, you'll need to start the \index{API}API running again in order to call it.
:::
...
In my app, I have inputs `bill_length`, `sex`, and `species`, which I collect
in a reactive called `vals()`. 
