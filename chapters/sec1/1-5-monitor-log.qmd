...
I decided to log when the
app starts, just before and after each request, and an error logger if
an HTTP error code comes back from the API. 

With the logging now added, here's what the app looks like in R:

``` {.r eval="false" include="../../labs/lab-5/app-log.R" filename="app.R"}
```

And in Python:

``` {.python eval="false" include="../../labs/lab-5/app-log.py" filename="app.py"}
```
...
Feel free to add whatever logging you want. 
With all that added, the code looks like this:
