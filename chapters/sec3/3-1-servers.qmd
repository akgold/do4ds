# Servers, Linux, and the Cloud {#sec-servers}
...
some version of Linux.[^3-1-servers-1]

[^3-1-servers-1]: There are also versions on Linux that run on desktop
    computers. But only nerds like me, and probably you, put Linux on
...
didn't actually include any code from Unix itself.[^3-1-servers-2]

[^3-1-servers-2]: Or at least they weren't supposed to. BSD is a Unix
    clone that was the predecessor of MacOS that has an interesting
...
welcome, but I won't promise I'll implement them :-)." [^3-1-servers-3]

[^3-1-servers-3]: More in the [History of Linux Wikipedia
...
majority of the world's servers run on Linux.[^3-1-servers-4] Almost all
of the world's embedded computers -- in ATMs, cars and planes, TVs,
...
[^3-1-servers-4]: The remainder are almost entirely Windows servers.
...
one.[^3-1-servers-5] Many organizations on AWS are using Amazon Linux,
which is independently maintained by Amazon but was originally a RHEL
derivative.[^3-1-servers-6]

[^3-1-servers-5]: RHEL and CentOS are related operating systems, but
...
[^3-1-servers-6]: As I'm writing this, Amazon Linux 2 is popular, but
...
If you're getting a server today, you're getting it from *the cloud*.

You've really only got one option -- buy a physical server, install it
somewhere, and host it.

Before you can start doing your work, you'll need to buy a physical
...
[^3-1-servers-7]: https://www.visualcapitalist.com/aws-powering-the-internet-and-amazons-profits/
...
provider.[^3-1-servers-8]

[^3-1-servers-8]: https://a16z.com/2021/05/27/cost-of-cloud-paradox-market-cap-cloud-lifecycle-scale-growth-repatriation-optimization/
...
unpredictable workloads, or for temporary workloads.[^3-1-servers-9]

[^3-1-servers-9]: By temporary workloads, I mean for example the need
    for a very large server to run a particular machine learning model.
    This is *not* the autoscaling failure case, because you're not
    expecting this server to turn itself on and off. Instead, you'd
    stand it up -- run it for a few hours, days, or weeks while you're
    using it, and then manually turn it off. This is super easy with a
    cloud server and very difficult in an on-prem world.
...
pre-installed or configured to make your life easier.[^3-1-servers-10]

[^3-1-servers-10]: There are also some wild services that do specific
...
| Database                 | RDS/Redshift[^3-1-servers-11]    | Azure Database                 | Google Cloud SQL               |
| ML Platform              | SageMaker                        | Azure ML                       | Vertex AI                      |

[^3-1-servers-11]: There are many others. These are just the most
    popular I've seen for data science use cases.
...
of advice on how to choose in [Chaper @sec-cloud-scale].
:::
...
### Step 5: Put the penguins data and model in S3

As discussed in [Chapter @sec-data-access], we're going to want to put
our data and our model somewhere other than on our local machine.
...
It'll look something like this[^3-3-file-users-3]

[^3-3-file-users-3]: Not my real access key or secret access key. Never
    share them.
