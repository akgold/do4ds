...
[^3-2-linux-admin-2]: People who are pedantic about operating systems or
...
|-------------------------------------|---------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
|                                     |                                                         |                                                                                                                  |
| `chm o d <pe r m i s sions> <file>` | Modifies permissions on a file.                         | Number indicates permissions for user, group, others: add 4 for read, 2 for write, 1 for execute, 0 for nothing. |
| `ch o wn <u s e r / group> <file>`  | Change the owner of a file.                             | Can be used for user or group. , e.g. `:my-group`.                                                               |
...
| Command                                   | What it does                                  |
|-------------------------------------------|-----------------------------------------------|
| `apt-g e t u p date && apt-get upgrade y` | Fetch and install upgrades to system packages |
...
|         |                            | with `head`.               |
+---------+----------------------------+----------------------------+
| `df`    | Check storage space on     | `-h`                       |
|         | device.                    |                            |
+---------+----------------------------+----------------------------+

[^3-2-linux-admin-9]: Note that if you're using `du` with `-h`, `sort`
    also needs a `-h` so it knows to sort by human-readable attributes.
    Otherwise the sorting gets weird.

[^3-2-linux-admin-10]: Note that if you're using `du` with `-h`, `sort`
    also needs a `-h` so it knows to sort by human-readable attributes.
    Otherwise the sorting gets weird.
...
`ps aux`.[^3-2-linux-admin-11] It lists all processes currently running
...
[^3-2-linux-admin-11]: This is another one where you'll almost never use
    `ps` without `aux`.
...
first.[^3-2-linux-admin-12]

[^3-2-linux-admin-12]: Alternatively, you could open a file of the right
...
the default name, probably `id_ed25519`.[^3-2-linux-admin-13] The `ssh`
command knows to automatically used a default key, so you no longer need
the `-i` option.

[^3-2-linux-admin-13]: The pattern is `id_<encryption type>`. `ed25519`
