...
On Mac or Linux, the entire filesystem is a tree (or perhaps an
upside-down tree). Every directory is contained in by a *parent
directory*, and may contain one or more *children* or *sub-directories*.
The *root directory, `/`* is the base of the tree and is its own parent.
A `/` in between two directories means that it's a sub-directory.
...
Each Linux computer has exactly one filesystem. On Windows you can have
multiple filesystems on the same machine, one for each physical or
logical disk. You're probably familiar with `C:` as your main
filesystem. Your machine may also have a `D:` drive.
...
|                     |                | notes          |
+=====================+================+================+
| `c h m o d < p      | Modifies       | Number         |
| ermissions> <file>` | permissions on | indicates      |
|                     | a file.        | permissions    |
|                     |                | for user,      |
|                     |                | group, others: |
|                     |                | add 4 for      |
...
|                     |                | nothing.       |
+---------------------+----------------+----------------+
| `c h o w n <        | Change the     | Can be used    |
| user/group> <file>` | owner of a     | for user or    |
|                     | file.          | group, e.g.    |
|                     |                | `:my-group`.   |
+---------------------+----------------+----------------+
...
| Command                       | What it does   |
+===============================+================+
| `a p t - g e t                | Fetch and      |
| update  && apt-get upgrade y` | install        |
|                               | upgrades to    |
|                               | system         |
|                               | packages       |
+-------------------------------+----------------+
...
|              |              | options      |
+==============+==============+==============+
| `netstat`    | See ports    | Usually used |
|              | and services | with `-tlp`  |
|              | using them.  |              |
+--------------+--------------+--------------+
...
Install Shiny Server using the instructions from the [Admin
Guide](https://docs.posit.co/shiny-server/#getting-started). Note that
you don't need to install R, as we've already done that, and you don't
need to install `{shiny}`, as we're going to use `{renv}`.

TODO

Edit `/etc/shiny-server/shiny-server.conf` to run the right app -- may
have to edit user.
...
Palmer Penguin container on 8080.
...
`8787`, JupyterHub on `8000`, and our Plumber API on `8080`. You can try
