...
+=======================================+=====================+==============================+
| `chmod <permissions> <file>`          | Modifies            | Number indicates permissions |
|                                       | permissions on a    | for user, group, others: add |
|                                       | file.               | 4 for read, 2 for write, 1   |
|                                       |                     | for execute, 0 for nothing.  |
+---------------------------------------+---------------------+------------------------------+
| `chown <user/group> <file>`           | Change the owner of | Can be used for user or      |
|                                       | a file.             | group, e.g. `:my-group`.     |
+---------------------------------------+---------------------+------------------------------+
...
| Command                                | What it does        |
+========================================+=====================+
| `apt-get update  && apt-get upgrade y` | Fetch and install   |
|                                        | upgrades to system  |
|                                        | packages            |
+----------------------------------------+---------------------+
...
+==============+==============+===================+
| `du`         | Check size   | Most likely to be |
|              | of files.    | used              |
|              |              | `du  -h           |
|              |              |  <dir> | sort -h` |
|              |              | [^3               |
...
In this lab, we're going to take your server from an empty server that
only you can access to one with users and useful software installed and
running -- even if it's not yet accessible to the outside world.
...
the permissions, you'll be unable to, and you'll get a warning that
...
system (try `ls ~/.ssh`). If you've got one already, I'd recommend just
using that key so you don't have to mess around with multiple keys.

If you don't have one, google how to create an SSH key and do it on your
laptop.
...
R on the GitHub repo -- https://github.com/r-lib/rig. Use those
instructions to install the current R release on your AWS server.
...
Once R is installed, let's download and install RStudio Server. This
should be a very easy process. The basic process is to install the
