...
package that's available PyPI, Conda Forge, CRAN, BioConductor, GitHub
whenever you want. Where a small or immature organization might have a
laissez-faire attitude toward packages, an enterprise will not. Instead,
most enterprises block software that hasn't explicitly been allowed.
...
IT/Admins's biggest concern about packages is that they might be unsafe.
For example, a package could introduce an exploitable bug in your code,
be a trojan horse that exfiltrates data when activated, or include
incorrect code that will yield numerically bad results.
...
in Javascript might show up in the version of Javascript used by \index{Jupyter
Notebook}, \index{RStudio}, \index{Shiny}, or Streamlit. Many companies disallow using
...
When someone releases software, they can choose a *license*, a legal
...
since there have been no court cases on this topic, nobody knows how it
would shake out if it does someday get to court, leading some
organizations to err on the side of caution.
...
\index{Jupyter Notebook} under a permissive modified BSD. R is released under
the copyleft GPL license and \index{RStudio} under a copyleft AGPL.
...
include Jfrog Artifactory, Sonatype Nexus, Anaconda Business, and \index{Posit}
Package Manager.

Artifactory and Nexus are generalized library and package management
solutions for all sorts of software, while Anaconda and \index{Posit} Package
Manager are more narrowly tailored for data science use cases. I'd
suggest working with your IT/Admins to get data science focused
repository software. Often these repositories can run alongside
general-purpose repository software if you already have it.
