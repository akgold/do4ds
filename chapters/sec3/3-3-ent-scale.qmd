...
## Size an enterprise cluster

Your IT/Admin team probably knows much about scaling a computational
cluster, but little about data scientists use computational resources.
They want to know how many machines, what size, and in what
configuration will best support your data science work. Here are some
mental models I've found helpful to communicate with IT/Admin teams
about scaling an enterprise cluster.

Usually, a data science organization has a *baseline load*. This
consists of everyday data science users working on the platform, jobs
running, and visitors coming to apps and reports. Aside from the total
load for this work, the IT/Admins will also need to know the minimum
node size to support a workload. For example, some organization could
run all their jobs on machines with 16 GB of RAM, but that would be
woefully inadequate for others.

Second, there's *burst usage*. This is extra capacity sometimes needed
to support additional users, jobs, or applications. For example, your
organization might occasionally do workshops that pull in more users
than usual, or you're launching a new data science app that will add
load to the cluster.
...
1.  Baseline number of CPU cores, amount of RAM, and the largest jobs
    you'll have to accommodate daily.
...
It will be unaffordable for most teams to keep all this capacity
continuously available, and a scaling strategy is needed.
...
When burst usage or special purpose machines are needed and the need is
for a discrete time (i.e., a day or longer), it's often easiest for an
IT/Admin to use IaC to stand up more machines temporarily. Creating a
standalone environment for special-purpose projects or manually
expanding the size of an existing cluster are often the simplest,
especially if your organization has good IaC practices.
...
rules and requirements might make it hard to stand up nodes, or easy to
get more nodes for an existing cluster but hard to get a new cluster.

Other organizations want to run only one cluster and want that cluster
to scale itself automatically. Some very competent IT/Admin teams can
create a single autoscaling data science cluster, but it's hard. I've
often found that organizations trying for a single cluster are besotted
with the elegance of the idea and underestimate the difficulty of making
it work. Unless you want to have a bad time, you may have to impress
upon your IT/Admins that running heterogeneous workloads in one cluster
isn't trivial and that a data science workbench is a particularly
awkward fit for the autoscaling frameworks they probably prefer.

Autoscaling to make a cluster bigger is never a problem, but things get
tricky when you try to scale down. Most autoscaling frameworks choose
the cluster size based on actual resource usage (usually CPU). But CPU
usage is very uneven for a data science environment, particularly a
workbench. Most activities in a data science IDE, like typing code,
debugging, or examining data, use minuscule amounts of CPU. Your brain
is cranking, but the CPU is not.

From the perspective of a CPU-based autoscaling framework, you're not
doing anything and it will want to rip the node out from under you. This
is an incredibly frustrating user experience. Autoscaling frameworks
that work well for a data science workbench need to have a notion of
active sessions that doesn't depend on actual resource usage. If your
IT/Admin wants to autoscale your data science workbench, this is the
number one thing you must impress on them, or you will have a bad time.

These days, autoscaling implies Kubernetes for many organizations.
Unfortunately, Kubernetes is an awkward fit for a heterogeneous data
science cluster for three reasons. First, Kubernetes pods are generally
...
Second, Kubernetes has powerful tooling to scale a cluster up and down
quickly. Unfortunately, it is usually CPU-based and you must trick the
Kubernetes scheduling engine to work well for a data science
environment. Last, Kubernetes allows for heterogeneous nodes (e.g.,
large or GPU-backed) in one cluster, but it's not a core use case and
can be awkward to configure.[^3-3-ent-scale-5]

[^3-3-ent-scale-5]: You, or your IT/Admin, will have to work with
    *taints*, *tolerations*, *node pool selectors*, *node affinities*,
    and more.

A better solution exists, though it's often overlooked -- *high
performance computing (HPC)* frameworks like *Slurm*, *LSF*, and *Grid
Engine*. HPC is that it allows users to address a cluster of many
machines as one. This allows creating supercomputers with thousands of
CPU cores, and so that's when people mostly think about HPC. But HPC
also works quite well at a more modest size, for example, to run a
workload requiring 30 GB of RAM in a cluster with only `t3.xlarge`s with
16GB of RAM each.

Even if you don't have any node sizing issues, HPC can be a great fit
for autoscaling or a cluster with heterogeneous nodes. HPC frameworks
are generally quite session-aware, making them a great choice for
autoscaling a data science workbench. Most also support different
categories of work (called *queues* in Slurm) right out of the box. If
you're interested in trying out Slurm, AWS has a great service called
...
The upshot is that it's easiest to create standalone data science
environments for special-purpose machines or manually scale a cluster.
Most IT/Admins will use a standard autoscaling framework or Kubernetes
if you need heterogeneous nodes or autoscaling. If possible, you should
explain the difficulties with autoscaling and suggest that HPC may be a
better solution.
