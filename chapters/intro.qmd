...
the background, and there's no \index{Google Sheet}, CSV file, or half-baked
...
Engineering team at \index{Posit} (then \index{RStudio}). The Solutions Engineering team
helps customers deploy, install, configure, and use \index{Posit}'s Professional
Products.
...
science products more robust with open-source tooling and \index{Posit}'s
...
predictions to another service from a machine learning model via an \index{API}.
...
using some form of \index{Agile}. \index{Agile} work patterns have also extended beyond
software into more general project management.

You may have heard of some of the dozens of \index{Agile} software development
...
download \index{Jupyter Notebook} or \index{RStudio}, install Python and R, and get to
work. However, organizations are increasingly centralizing data science
operations onto a centralized *data science platform* or *data science
environment*.
...
instance where we can do this work. It will include \index{RStudio Server} and
\index{JupyterHub} for working. It will also host the machine learning model as
an \index{API} and the \index{Shiny} app for the website.
