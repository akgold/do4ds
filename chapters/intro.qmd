...
the background, and there's no \index{Google Sheet}, CSV file, or
...
Engineering team at \index{Posit} (then \index{RStudio}). The Solutions
Engineering team helps customers deploy, install, configure, and use
\index{Posit}'s Professional Products.
...
application programming interface (\index{API}).
...
giving a name to this new software development philosophy. \index{Agile}
development ate the world. Essentially all software is now developed
using some form of \index{Agile}. \index{Agile} work patterns have also
extended beyond software into more general project management.

You may have heard of some of the dozens of \index{Agile} software
...
complemented \index{Agile} software development.
...
download \index{Jupyter Notebook} or \index{RStudio}, install Python and
R, and get to work. However, organizations are increasingly consolidating
data science operations onto a centralized *data science platform* or
...
[Part 1](@sec-1-intro) is about applying DevOps best practices to a
data science context. Adhering to these best practices will make it
easier to take projects into production and ensure their security and
stability once they're there. While these best practices are inspired by
DevOps, data science and data science projects are different enough from
...
[Part 3](@sec-3-intro) is about how everything you learned in Part
2 is inadequate at organizations that operate at enterprise scale. If
Part 2 explains how to do IT/Admin tasks yourself, Part 3 is my
...
Note how every node is a noun (or pronoun), and the edges (labels on the arrows) are
...
instance where we can do this work. It will include
\index{RStudio Server} and \index{JupyterHub} for working. It will also
host the machine learning model as an \index{API} and the \index{Shiny}
app for the website.
...
Throughout the book, I will *italicize* both the names of other publications and terms of art when introducing
them. Because so many of the
