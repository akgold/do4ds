...
log in again, allowing token lifetimes to be very short. That drastically
...
PAM can also be used to do things when users log in -- the most common
...
often used to automatically fetch a ticket upon user log in.
...
[^auth-3]: The diagram below assumes you don't already have a token in
    your browser. Step 6 is actually the first thing that happens when
    the user visits the SP. If the user has a token already, steps 2-5
    get skipped.
...
services across the web. Any time you've used a *Log in with
