...
|                    |                    | for a live view.   |
+--------------------+--------------------+--------------------+
| `g r               | Search a file      | Writing regex can  |
|  e p <expression>` | using regex.       | be a pain. I       |
|                    |                    | suggest testing    |
|                    |                    | expressions on     |
|                    |                    | regex101.com.      |
|                    |                    |                    |
...
+--------------------+-------------------------------------------------+
| `u                 | Modify user `username`. `-aG <group>` adds to a |
| sermod <username>` | group                                           |
+--------------------+-------------------------------------------------+
...
|                       | file.            | group, others: add 4 for |
|                       |                  | read, 2 for write, 1 for |
|                       |                  | execute, 0 for nothing.  |
+-----------------------+------------------+--------------------------+
| `chown                | Change the owner | Can be used for user or  |
|  <user/group> <file>` | of a file.       | group, e.g. `:my-group`. |
+-----------------------+------------------+--------------------------+
...
| `useradd`      | Add a new user.                              |
+----------------+----------------------------------------------+
| `use r m       | Modify user `username`. `-aG <group>` adds   |
| od <username>` | to a group                                   |
+----------------+----------------------------------------------+
...
| own <us e r /  | owner of a     | user or group,     |
| group> <file>` | file.          | e.g. `:my-group`.  |
+----------------+----------------+--------------------+
| `              | Change active  |                    |
| su <username>` | user to        |                    |
|                | `username`.    |                    |
...
| **Command**              | **What it does**          |
+--------------------------+---------------------------+
| `apt-get u p d a t       | Fetch and install         |
| e && apt-get upgrade -y` | upgrades to system        |
|                          | packages                  |
+--------------------------+---------------------------+
| `a p                     | Install a system package. |
| t-get install <package>` |                           |
+--------------------------+---------------------------+
...
+----------------------+-------------------------------------+
| `ln -s < lo          | Creates a symlink from file at      |
| cation to link>: < l | `location to link` to               |
| ocation of symlink>` | `location of symlink`.              |
+----------------------+-------------------------------------+
...
system.[^3-10-troubleshoot-1]

[^3-10-troubleshoot-1]: This command actually cuts off the header line
    of the table. What I actually ran was
    `ps aux | grep "RStudio\|USER"`.
