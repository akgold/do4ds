...
In [Chapter @sec-networking] you learned all about how IP Addresses
are where a host actually lives on a computer network. But you've been
using the internet for a long time and you've rarely -- if ever --
actually used an IP Address. What gives?
...
When you create or launch a website, you'll purchase a domain like
$\text{do4ds.com}$. Purchasing a domain (renting one, really) gives you
the right to attach that domain to an IP Address.

Then, when someone else goes to visit your website, they'll put that
domain in their browser, and the DNS system will properly return the IP
Address you've associated with the domain. Registering your purchase of
the domain and its association to a particular IP Address is the
responsibility of the *domain name registrar*, which are the companies
that actually own the domains.
...
When you go to get a domain from a registrar, the thing you'll actually
rent is the root domain.

Domain name servers are separated by top-level domains, and domain names
are only unique within a given top-level domain. So a DNS lookup for
$\text{example.com}$ starts somewhere completely different than
$\text{example.org}$ and they could be completely different websites
...
## DNS lookups return IP Addresses

When you go to visit a website, your computer *resolves* the IP Address
of where to send the traffic via a *DNS lookup*. The basic idea of DNS
resolution is that your computer knows where to find the DNS
nameservers, which return the correct IP Address.

The issue is that DNS resolution happens so often -- and it needs to be
fast and resilient. In order to make sure this is the case, DNS queries
are routed through several different layers of decentralized
nameservers.[^2-7-dns-2]

[^2-7-dns-2]: The big ones are that your computer actually never
    directly talks to nameservers. Instead, it talks to a server called
    a *resolver*, which is the server that talks to the nameservers. And
    once it gets there, there are several layers of nameserver that it
    might go through before it gets to the right record.

In order to make things even faster, the results of a DNS lookup are
cached at a number of different places along the way, including on your
computer.
...
get routed to, and many of them may have outdated cache entries. That's
why DNS changes can take up to 24 hours to propagate.
...
Configuring DNS can be somewhat opaque. Luckily, most places you might
serve a website from have instructions on how to do it you can google.
The point of this section is just to make you comfortable with whatever
instructions you find in your googling.

Costs for domain names vary widely. Buying a meaningless domain in a
less popular top-level domain, say $\text{ladskfmlsdf.me}$, can cost as
little as \$3 per year. On the other hand, buying a $\text{.com}$ domain
that's a real word or phrase can be thousands of dollars. There are
articles every few years about some major company accidentally allowing
their domain name to lapse and ransoming it back for stupid amounts of
money.
...
different categories, but there are only two you'll probably need and
one more you might see:

-   *A records* (or their IPv6 cousin *AAAA records*) map a domain to an
    actual IP Address.
-   *CNAME records* alias sub-domains to another record.
...
The first row is A record to go to the target IP Address. The path
$\text{@}$ is a special symbol meaning exact match. So by this
configuration, any traffic to $\text{example.com}$ will be passed
straight through to the specified IP Address.
...
might be owned by a completely different group than the main website.
...
Most DNS configuration services also allow you to configure the *TTL
(time to live)*. This tells any service that fetches the DNS record how
long to cache the result for. You can manually configure this if you
want, but I generally just stick with the defaults.
...
is that every time the server stops, that IP is released and a new one
is assigned when it comes back up. This means you'd have to change your
DNS record every time you temporarily take your server offline -- no
good.
...
$\text{do4ds-lab.shop}$ for \$1.98 for a year on namecheap.com, which
has long been my choice for purchasing domains because it is indeed,
cheap.
...
In NameCheap, you configure this via a table under Advanced DNS, which
looks like this.
