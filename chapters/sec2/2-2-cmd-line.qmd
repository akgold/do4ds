...
around the command line. There are a variety of tasks that are quicker
and easier that way. Most people who learn the command line end up using
it a lot on their personal computers.
...
command line or are using an intermediary tool to limit what admins can
do. Either way, it's going to be harder to get things done.
:::
...
shell (SSH)*, the side door you'll use to do administrative tasks on
your server.
...
best use of your time and energy. Those people are no fun.
...
line on your machine. *SSH* -- short for *Secure (Socket) Shell* -- is
the program that allows direct interactions with a server's command line
from the command line of your computer.[^2-2-cmd-line-2]

[^2-2-cmd-line-2]: SSH actually can be used between **any** two
    machines, provided at least one is accepting SSH connections. For
    data scientists, it's almost always used to connect from a local
    machine to a server.
...
For example, connecting to the server at `server.example.com` as the
user `alex` from your computer (*local host*) would look like
...
terminal of the server. This session is authenticated and secured via
the exchange of *SSH keys*.

In order to make this all work, you'll have to configure your *SSH
keys*, which come in a set called *keypair*. Each keypair consists of a
*public* key and a *private* key. You'll register your private key
anywhere you're trying to SSH to, like a server or git host, but your
private key must be treated as a precious secret.

Once you've registered your public key and youuse the `ssh` command,
your local machine sends a request to open an SSH session to the remote
and includes the private key with the request.

The remote host verifies the private key with the public key and open an
...
The reason this all works is that, it's easy to check whether a private
key is the right one for the public key you've got. But if you only have
a public key, it's nearly impossible to fabricate the private key that
goes with it.

Honestly, I wish public and private keys were named differently. Calling
the private key the *key* and the public key the *lock* makes the intent
...
keypairs. One on your personal computer to SSH to the server, and once
on the server to access outside services that use SSH, like GitHub.
...
default name, probably `id_ed25519`.[^2-2-cmd-line-3] If you're only
creating one key on each machine, I'd recommend sticking with the
default name because the `ssh` command will automatically uses keys with
the default name.

[^2-2-cmd-line-3]: The pattern is `id_<encryption type>`. `ed25519` is
...
Many people also like using the *tmux* command line utility with SSH.
SSH sessions block the terminal session they're using and will break
when your computer goes to sleep.

tmux is a terminal multiplexer, which allows you to manipulate terminal
sessions from the command line, including making sessions durable
through sleeps and other operations. To be honest, I'm mentioning tmux
because lots of people love it, but I've tried using it and found the
learning curve too steep for it to come into regular usage for me. Your
mileage may vary.
