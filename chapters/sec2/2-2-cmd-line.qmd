...
The `.pem` key you downloaded is the skeleton key to your server. You should be extremely careful with the `.pem` key. In [Chapter @sec-linux-admin], we'll set up an actual user on the server and configure SSH, which is more secure.
