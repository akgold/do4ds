...
HTTP traffic is sent in plaintext, which means it's actually like
...
protocol, but it does secure the whole process.
...
the certificate is then used to verify the site's identity and establish
...
exactly the same, so most people still talk in terms of SSL when they
really mean TLS.
:::
...
have been neutered. Because of HTTPS it's now safe to use any random
WiFi network you want.
...
certificates. While it was only \$10 per year to for a basic SSL
...
as creating an SSH key. But using a self-signed certificate is a pain
because you have to manually add the public certificate to the CA store
of every machine that accesses the site, and then re-add it when the
certificate expires.[^2-8-ssl-4]
...
to create and configure your certificate right on your server.

Most organizations, use a public CA to get SSL on public-facing
...
### Step 2: Let \index{RStudio Server} know it's on HTTPS

Before we exit and test it out, let's do one more thing. As mentioned
when we configured NGINX the first time, \index{RStudio Server} does a bunch of
proxying traffic back to itself, so it needs to know that it's on HTTPS.

You can add a header to all traffic letting \index{RStudio Server} know the
protocol is HTTPS by adding this line to your `nginx.conf`:

``` {.bash filename="/etc/nginx/nginx.conf"}
proxy_set_header X-Forwarded-Proto https;
```

Ok, now try to visit \index{RStudio Server} at your URL and you'll find
...
security group so there are only 2 rules -- one that allows SSH traffic
on $22$ and one that allows HTTPS traffic on $443$.
