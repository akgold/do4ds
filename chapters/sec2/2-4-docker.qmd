...
When you install Docker on your system -- whether desktop or server --
you also install the `docker` command line utility. This command line
utility is how you build, run, and manage containers. Here are a few of
the most commonly used `docker` sub-commands.
...
**Frequently used `docker run` flags**
...
| `-p <host port:container port>` | Publish port from inside container to outside. | Needed if you want to access anything inside the container with HTTP.                                                                                            |
| `-v <host dir:container dir>`   | Mount volume into the container.               | Make a directory (volume) on the host available to the container for reading and writing.                                                                        |
...
pretty much free-reign over your system, so that's when you'll do things
...
Now, thinking back to [Chapter @sec-data-arch], we're going to run into some trouble when when and if we move the API off of our laptop and onto a server. We'll address that in [Chapter @sec-cloud].
