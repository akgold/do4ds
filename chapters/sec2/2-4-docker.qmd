...
``` {.Dockerfile eval="false" include="../../labs/lab-6/Dockerfile" filename="Dockerfile"}
```
...
``` {.Python eval="false" include="../../labs/lab-6/app.py" filename="app.py"}
```
...
That's right -- the temp file indicated is inside the `/var` directory on *my machine*, but that directory doesn't exist at `/var` inside the
...
::: callout-tip
Depending on the permissions of the `/var` directory on your machine, you *may* need to run the `docker run` command prefixed by `sudo`.
:::
