...
You're accustomed to interacting with a computer and a phone running
MacOS or Windows or iOS or Android. But most servers don't run any of
those.
...
computers run Linux -- in ATMs, cars and planes, TVs, smart thermostats,
and most other gadgets and gizmos. Android is a version of Linux, as is
ChromeOS. Almost all of the world's supercomputers run Linux.

Most importantly for our purposes, the majority of the world's servers
...
That means in order to administer a server, you're going to have to
learn a little about Linux. In this chapter, you'll learn about the
history of Linux and about how to navigate and manipulate a server
running Linux.
...
looked nothing like it does today. Computers of that era had extremely
tight linkages between hardware and software. There was no Microsoft
Word you could use on a Dell machine or an HP machine or an Apple
machine.
...
middleware that reduced the need for coordination between computer
manufacturers and software developers. Computer manufacturers could
design computers that ran Unix and developers could write software that
ran on Unix.
...
Obviously, the Linux project outgrew that modest newsgroup post.
Currently, there are over 600 *distros* (short for distribution) of
Linux, which differ by technical attributes and licensing model. The
number of distros reflects both the natural fragmentation of popular
open-source projects, as well as the disparate requirements for systems
as varied as your car's infotainment system, a smartphone, and the
controller for your smart thermostat.
...
[^2-3-linux-5]: As I'm writing this, Amazon Linux 2 is popular, but
    ...
    or its successor to be dominant by the time you read this.

Most individuals who have a choice in the matter, including me, prefer
Ubuntu. It's a little simpler and easier to configure than the others.
...
(LTS) releases, which are released in April of even years.
...
are running Focal (20.04, Focal Fossa) or Jammy (22.04, Jammy
Jellyfish).
:::
...
which sort of makes sense. Over time, you'll get very comfortable with
...
Bash is a fully functional programming language, so you can assign
...
Whenever a program is running, it is running as a particular user who
can be identified by their *username*.

On any Unix-like system, the `whoami` command returns the username of
the active user. When I run `whoami` on my MacBook, I get:
...
command. On my MacBook, I'm a member of several different groups, with
the primary group `staff`.
...
Determining whether a user can take an action is really a question of
...
example, a user could have read permissions on a directory so they could
see the names of the files, but not actually have read permissions on
any of the files so they can't look at the contents.
...
::: callout-note
If you spend any time administering a Linux server, at some point you
almost certainly will find yourself applying `chmod 777` to a directory
or file to rule out a permissions issue.

I can't tell you not to do this -- we've all been there. But if it's
something important, be sure you change it back once you've figured
things out.
:::

In some cases, you might actually want to change the owner or group of a
file. You can change users and groups with the `chown` command. Changing
users just involves the username and changing groups get prefixed with a
colon.
...
the *root directory*, `/`. The rest of the filesystem is a tree (or
perhaps an upside-down tree). Every directory is contained in by a
*parent directory* and may contain one or more *children* or
...
operating system called BSD that, like Linux, is a Unix clone. If you're
familiar with Windows, the Linux filesystem may seem a little strange.
...
directory*, which usually live inside `/home`.
...
paths with `cd`. If you were in `/home/alex` and wanted to navigate to
`/home`, either `cd ..` or `cd /home` would work.
...
your desktop or vice versa. There are a few different ways to move files
and directories.

If you're moving multiple files, it's easier to first combine them into
a single object. The `tar` command turns a set of files or whole
directory into a single archive file, usually with the file suffix
...
unpack (extract) archive files, but telling it which one requires the
use of several flags. You'll basically never use `tar` without a bunch
of flags and the incantation is hard to remember. I google it every time
I use it. The flags you'll use most often are in the cheatsheet in
...
key for them so you can directly login from your personal computer.
...
::: callout-tip
We're copying the public key, but SSH access (argument to `-i`) is still
with the server's `.pem` key because there isn't another one registered
yet.
:::
...
safe and never use it to login again.
