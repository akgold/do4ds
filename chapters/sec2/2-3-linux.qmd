...
*Linux* is the world's predominant *operating system (OS)* outside of
personal computers and phones. Almost all of the world's embedded
...
Most importantly for our purposes, most of the world's servers also run
...
navigate and manipulate a server running Linux.
...
on a machine from Dell or HP or Apple.
...
Most individuals who have a choice, including me, prefer Ubuntu. It's a
...
run Focal (20.04, Focal Fossa) or Jammy (22.04, Jammy Jellyfish).
:::
...
assigned when the user is created and usually don't need to be changed
...
themselves and run as those users. For example, installing \index{RStudio
Server} will create a user with username `rstudio-server`. Then, when
\index{RStudio Server} goes to do something -- start an `R` session for example
...
The `id` command shows a user's username, `uid`, groups, and `gid`. On
my MacBook, I'm a member of several groups, with the primary group
`staff`.
...
Basic Linux permissions (*POSIX permissions*) consist of a 3x3 matrix of
read, write, and execute for the owner, the owning group, and everyone
...
familiar with Windows, the Linux filesystem may seem strange.
...
flags you'll most often use are in the cheatsheet in [Appendix
@sec-append-cheat].

You can move files to or from a server with the `scp` command. `scp`
(short for *secure copy*) is `cp`, but with an SSH connection in the
middle.[^2-3-linux-9]
...
regular terminal on your laptop, not one already SSH-ed into your
server.
...
`{magrittr}` pipe was inspired by pipe operators from Unix (Linux) and
...
4.  How can you copy, move, or delete a file? What about to or from a
    server?
5.  Create a mind map of the following terms: Operating System, Windows,
    MacOS, Unix, Linux, Distro, Ubuntu
6.  What are the 3x3 options for Linux file permissions? How are they
