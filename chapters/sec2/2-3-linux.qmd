...
[^2-3-linux-1]: The remainder are almost entirely Windows servers. There
...
`cp`, but with an SSH connection in the middle.[^2-3-linux-8]
...
[^2-3-linux-9]: The title of this callout box is also the tagline for
    the `{magrittr}` package. When you have existing text you want to
    add to a file, you can use the `>` command, which takes the output
    of a command on the left and writes it as a new file. If the file
...
create a new file if one doesn't exist, and I usually don't meant to
...
`echo .Rprofile >> .gitignore`.[^2-3-linux-10]

[^2-3-linux-10]: Note that `echo` is needed so that the `.Rprofile` gets
    repeated as a character string. Otherwise `.Rprofile` would be
