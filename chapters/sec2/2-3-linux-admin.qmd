...
You'll need to `scp` the public key to the server first.[^2-3-linux-admin-7]

[^2-3-linux-admin-7]: Yes, you could just copy/paste the contents of your public key into the public key in the authorized_keys directory, but this is a good chance to practice using `scp`.

    scp -i do4ds-lab-key.pem \\ \~/.ssh/id_ed25519.pub \\ #default name for ed25519 encrypted key ubuntu\@ec2-54-159-134-39.compute-1.amazonaws.com:/home/ubuntu
...
As of this writing, for Ubuntu 22.04, that will look something like
...
This will take a few moments to download and install. By default, this installs into the `/opt/R` directory. Once it's done, you can run R with `/opt/R/{R_VERSION}/bin/R`. Great!

You can also use the `rig` command line utility to install and manage versions of R. This is a little less manual.
...
Access RStudio Server using laptop browser at localhost:8787, use username `test-user` and password you set on the server. Because you're port forwarding, the browser on your laptop is actually connected to your server, but via SSH. We'll configure to connect directly without SSH in [Chapter @sec-networks-1].

RStudio Server runs as a system process -- R also needs to be installed.

### Installing JupyerHub + JupyterLab

There is a python install, but it's very barebones (e.g. no pip). You want a separate install for data science purposes.

sudo apt install python3.10-venv

Jupyter-specific venv: sudo python3 -m venv /opt/jupyterhub

Install jupyterhub: sudo /opt/jupyterhub/bin/python3 -m pip install wheel jupyterhub jupyterlab ipywidgets

Follow instructions here: https://docs.rstudio.com/resources/install-python/

Make sure to add to path and make available as Jupyter Kernel.

Install npm `sudo apt-get install npm`.

Follow instructions: https://jupyterhub.readthedocs.io/en/1.2.0/installation-guide-hard.html -- run all as sudo, make sure to use /opt/python.

`sudo /opt/python/3.7.13/bin/python -m pip install jupyterhub notebook`

JupyterHub runs *as* as python process.

Tunnel in

`ssh -L 8000:localhost:8000 test-user@$SERVER_ADDRESS`

Note if you ping `$SERVER_ADDRESS:8000` it doesn't work.
