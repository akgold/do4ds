...
be a member of secondary groups.[^2-3-linux-admin-6] By default, each
user's primary group is the same as their username.

[^2-3-linux-admin-6]: Depending on your version of Linux, there *may* be
...
group.[^2-3-linux-admin-7] Let's learn how to read these.

[^2-3-linux-admin-7]: You can generally ignore the number, which is the
...
[^2-3-linux-admin-8]: Clever eyes may realize that this is just the base
...
|                |                |                            |
+----------------+----------------+----------------------------+
| `chm           | Modifies       | Number indicates           |
| od <pe r m i s | permissions on | permissions for user,      |
| sions> <file>` | a file.        | group, others: add 4 for   |
|                |                | read, 2 for write, 1 for   |
|                |                | execute, 0 for nothing.    |
+----------------+----------------+----------------------------+
| `ch            | Change the     | Can be used for user or    |
| own <u s e r / | owner of a     | group. , e.g. `:my-group`. |
| group> <file>` | file.          |                            |
+----------------+----------------+----------------------------+
| `              | Change active  |                            |
| su <username>` | user.          |                            |
...
+==============+==============+============================+
| `du`         | Check size   | Most likely to be used     |
|              | of files.    | `du -h <dir> | sort -h`    |
|              |              | [^2-3-linux-admin-9]       |
|              |              |                            |
...
[^2-3-linux-admin-9]: Note that if you're using `du` with `-h`, `sort`
    also needs a `-h` so it knows to sort by human-readable attributes.
    Otherwise the sorting gets weird.
...
`ps aux`.[^2-3-linux-admin-10] It lists all processes currently running
...
[^2-3-linux-admin-10]: This is another one where you'll almost never use
    `ps` without `aux`.
...
detail in [Chapter @sec-networks-1]. For now, it's important to be able
to see what's running on your server that is accessible from the outside
...
| Command        | What it does    | Helpful options          |
+================+=================+==========================+
| `netstat`      | See ports and   | Usually used with `-tlp` |
|                | services using  |                          |
|                | them.           |                          |
+----------------+-----------------+--------------------------+
...
### Step 2: Add an SSH Key for your user

In order to be able to connect as a new user, we need to add the *public
key* as an authorized key for this user. If you're the server admin,
you'll have your users create their SSH keys, share the public keys with
you, and you'll put them into the right place on the server.

In this case you're both the user logging in and the admin, so you'll
just add your own SSH key in your own home directory.

We do so by putting the public key into the user's
`.ssh/authorized_keys` directory inside their home directory.

If you've already got an SSH key you use, feel free to use that one. If
you don't have one, google how to create an SSH key and do it on your
laptop.

You'll need to `scp` the public key to the server
first.[^2-3-linux-admin-11]

[^2-3-linux-admin-11]: Alternatively, you could open a file of the right
    name and just copy/paste the contents of your public key in there.
    Either way works.

    > scp -i do4ds-lab-key.pem   \~/.ssh/id_ed25519.pub  
    > ubuntu\@ec2-54-159-134-39.compute-1.amazonaws.com:/home/ubuntu

Note that we have to specify that we're using the `pem` key for the
server, which file we're copying, which user we're connecting as, and
where it's going on the server.

Now our public key is on the server, but it's in the `ubuntu` user's
home directory, it's owned by the `ubuntu` user, and it has the wrong
permissions.
...
When we used the `pem` key, we needed to specify it with `-i`. If you
followed standard instructions for creating a key, it was generated with
the default name, probably `id_ed25519`.[^2-3-linux-admin-12] The `ssh`
command knows to automatically used a default key, so you no longer need
the `-i` option.

[^2-3-linux-admin-12]: The pattern is `id_<encryption type>`. `ed25519`
    is the standard SSH key encryption type as of this writing.

If you want to use a different key name or if you have multiple SSH keys
for different services, you can set up a config file that maps addresses
to keys so you don't always have to use `-i`. A google search should
return good instructions for setting up your SSH config.
...
Posit makes [R binaries](https://docs.posit.co/resources/install-r/)
available for a variety of different operating systems, including Ubuntu
and RedHat. You can find instructions at
https://docs.posit.co/resources/install-r/.

TODO: confirm that installing into `/opt/R` works for RStudio Server
OSS.
:::
...
But knowing it's good to go isn't nearly as fun as actually trying it.
...
By default, RStudio Server is on port `8787`, so we'll tunnel port
`8787` on the server to `localhost:8787` on our laptop. The command to
do that is `ssh -L 8787:localhost:8787 test-user@$SERVER_ADDRESS`.

Now, if you go to `localhost:8787` in your browser, you should be able
to access RStudio Server and login with the username `test-user` and
password you set on the server.
...
Now that we've installed Python, we can create a standalone virtual
...
``` {filename="/etc/systemd/system/jupyterhub.service"}
[Unit]
...
## Running a Plumber API in a Container
...
This was easy to get something up and running quickly. But you should
notice that this isn't daemonized -- if we restart the server or the
container dies for any reason, it won't auto-restart.
