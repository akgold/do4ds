...
science applications like \index{JupyterHub}, \index{RStudio}, R, Python, and more.
...
files*. For applications hosted inside a \index{Docker} \index{Container}, behavior is
...
path every time. Some applications rely on others, like \index{RStudio Server}
needing to find R or \index{Jupyter Notebook} needing your Python kernels.
...
For example, on a data science workbench, you'd want \index{JupyterHub} and/or
\index{RStudio Server} to run as a service.
...
### Running \index{Docker} Containers as a service

People love \index{Docker} Containers because they easily run on most machines.
To run a \index{container} as a service, you'll need to make sure \index{Docker} itself
is daemonized and then ensure the \index{container} you care about comes up
whenever \index{Docker} does by setting a *restart policy* for the \index{container}.

However, many \index{Docker} services involve coordinating more than one
\index{container}. If so, you'll want to use a purpose-built system for managing
multiple containers. The most popular are *\index{Docker} Compose* or
*Kubernetes*.

\index{Docker} Compose is a relatively lightweight system that allows you to
...
of \index{Docker} Containers.

\index{Docker} Compose is fantastic for prototyping systems of \index{Docker} Containers
and for running small-scale Dockerized deployments on a single server.
There are many great resources online to learn more about \index{Docker}
Compose.
...
\index{Docker}-based workloads across a cluster of many servers.

In general, I'd recommend sticking with \index{Docker} Compose for the work
...
pre-built binaries provided by \index{Posit}.
...
recommend installing into `/opt/R` with binaries provided by \index{Posit} or
...
versions use \index{Docker} Containers or Linux *Environment Modules* to keep
...
we need to run a data science workbench and get our \index{API} and \index{Shiny} app
set up.
...
\index{JupyterHub} and JupyterLab are Python programs, so we will run them from
...
Now we will get \index{JupyterHub} up and running inside the virtual environment
we just created. \index{JupyterHub} has great docs (Google "\index{JupyterHub}
quickstart") to get up and running quickly. If you must stop for any
reason, assume sudo and start the \index{JupyterHub} virtual environment we
...
### Step 4: Daemonize \index{JupyterHub}

Because \index{JupyterHub} is a Python process, not a system process, it won't
automatically get daemonized, so we'll have to do it manually.

We don't need it right now, but it will be easier to manage \index{JupyterHub}
...
\index{JupyterHub} config (Google for the command), and move it into
`/etc/jupyterhub/jupyterhub_config.py`.

Now let's move on to daemonizing \index{JupyterHub}. To start, kill the existing
\index{JupyterHub} process (consult the cheatsheet in [Appendix
@sec-append-cheat] if you need help). Since \index{JupyterHub} wasn't
...
specifies that \index{JupyterHub} should use the config we just created,
specified by `-f /etc/jupyterhub/jupyterhub_config.py`.

Now, you'll need to use `systemctl` to reload the daemon, start
\index{JupyterHub}, and enable it.

### Step 5: Install \index{RStudio Server}

You can find the commands to install \index{RStudio Server} on the \index{Posit}
...
Unlike \index{JupyterHub}, \index{RStudio Server} daemonizes itself right out of the
box, so you can check and control the status with `systemctl` without
further work.

### Step 6: Run the penguin \index{API} from \index{Docker}

First, you'll have to ensure that \index{Docker} is available. It can be
installed from `apt` using `apt-get install docker.io`. You may need to
adopt `sudo` privileges to do so.

Once \index{Docker} is installed, running the \index{API} is almost trivially easy using
the command we used in [Chapter @sec-docker] to run our \index{container}.
...
### Step 7: Put up the \index{Shiny} app

We will use \index{Shiny} Server to host our \index{Shiny} app on the server. Start by
...
1.  Open R or Python and rebuild the package library with `{renv}` or
    `{venv}`.
2.  Install \index{Shiny} Server using the instructions from the \index{Shiny} Server
    Admin Guide.
    -   Note that you can skip steps to install R and/or Python, and the
        `{shiny}` package since we've already done that.
3.  Edit \index{Shiny} Server's configuration file to run the right app.
4.  Start and enable \index{Shiny} Server with `systemctl`.
...
First, we haven't daemonized the \index{API}. Feel free to try \index{Docker} Compose or
set a restart policy for the \index{container}.

Second, neither the \index{API} nor the \index{Shiny} app will automatically update when
we change them. You might want to set up a GitHub Action to do so. For
\index{Shiny} Server, you'll need to push the updates to the server and then
restart \index{Shiny} Server. For the \index{API}, you'd need to configure a GitHub
action to rebuild the \index{container} and push it to a registry. You'd then
need to tell \index{Docker} on the server to re-pull and restart the \index{container}.

Finally, there's no authentication in front of our \index{API}. The \index{API} has
limited functionality, so that's not a huge worry. But if you had an \index{API}
with more functionality, that might be a problem. Additionally, someone
could try to flood your \index{API} with requests to make it unusable. The most
common way to solve this is to buy a product that hosts the \index{API} for you
or to put an authenticating proxy in front of the \index{API}. We'll be adding
