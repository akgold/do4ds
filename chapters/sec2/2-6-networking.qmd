...
If you've got a server that no one can access, is it really there? AWS
would say yes, and they will still bill you. But that server won't do
you much good.
...
website, print something, or call an \index{API}, it sends network traffic and
waits for a response. That traffic is addressed to a particular location
and a particular *service*, which is the program that will do something
with that incoming traffic.

To understand how this all works, let's talk about the mail, i.e., the
real kind that goes in an envelope and is delivered in trucks. I imagine
our server as like an apartment building. Each service lives in one
...
(*Uniform Resource Locator)*.[^2-6-networking-1] A URL has four parts
that fully specify the network location of a
resource.[^2-6-networking-2] A full URL looks like this:
...
::: callout-note
Your computer gets the right IP Address from the domain using the
*Domain Name Service (DNS)*, which you'll learn about in [Chapter
@sec-dns]
:::
...
$\text{localhost}$ or loopback. $\text{localhost}$ is how a machine
sends traffic back to itself.

For example, if you open a \index{Shiny} app in \index{RStudio} Desktop,
the app will pop up in a little window along with a notice that says

``` {.bash filename="Terminal"}
Listening on http://127.0.0.1:6311
```

That means that the \index{Shiny} app is running on the same computer
and is available on port $6311$. You can open that location in your
browser to view the app as it runs.
...
the right mailbox in the building's mail room. Every port is uniquely
identified by a number. There are over 65,000 ports on every computer.
Since you're probably running no more than a handful of services, the
overwhelming majority of the ports are closed at any given
time.[^2-6-networking-4]
...
For example, maybe you have \index{RStudio} and \index{JupyterHub}
running on the same server. If you run them on their default ports,
\index{RStudio} will be on port $8787$ and \index{JupyterHub} will be on
$8000$.[^2-6-networking-5] Somehow you've got to smoothly redirect user
traffic, which comes in on $80$ or $443$, to those other ports. The most
common configuration is to use a piece of technology called a *proxy* to
put each service on a path. For example, you might configure the path
$\text{/rstudio}$ to go to port $8787$ on the server and the
$\text{/jupyter}$ path to go to $8000$.
...
In many cases, including using an EC2 instance, simply opening port $80$
or $443$ on the server or proxy still won't allow you to access the
server from the web. That's because there's a *firewall* sitting in
front of the server which blocks traffic to all but certain ports.
Before accessing anything on the server, you'll need to configure the
firewall to allow traffic on port $80$ or $443$.
...
For example, in this book, you've already seen a lot about SSH which is
an application layer protocol for allowing secure login and
communication over an unsecured network. SSH defaults to port $22$. Some
protocols piggyback on others. For example, SFTP for file transfers
secured with SSH also use $22$, and websockets, which are used by
\index{Shiny} and Streamlit, use standard HTTP(S) ports.
...
cheatsheets in [Appendix @sec-append-cheat].
...
This can be really handy if things a website is loading slowly or if
you're not sure why a page isn't loading. By inspecting the status codes
of different HTTP calls, the headers on those calls, and the time they
take, you can develop a pretty good idea of where things might be
getting stuck.

### SSH tunneling/port forwarding

When you start a new EC2 instance in AWS, the default security group has
only port $22$ open, allowing only SSH traffic. So far, you've seen SSH
used to access the command line on that remote server, but SSH can
actually be used to access any port in a process called *tunneling* or
*port forwarding*.
...
[^2-6-networking-6]: As you might guess from this complicated syntax,
    you can do a lot more than this with SSH tunneling; this is what I
    use it for most frequently.

So, for example, if your server were running at $64.56.223.5$ and you
have the SSH user `test-user`, you might forward \index{JupyterHub} on
port $8000$ with `ssh -L 8000:localhost:8000 test-user@64.56.223.5`.
Once the tunnel was established, you could access JupyerHub in your
browser on $\text{localhost:8000}$.
...
simple status report rather than the full webpsage, is useful.
...
but the path or port is incorrect. If you're running inside a \index{container},
you should check that you've properly configured the port inside the
\index{container} to be forwarded to the outside.
...
3.  Draw a mind map of trying to access the following in your browser.
    ...
    protocol
    1.  A \index{Shiny} app on a server at $\text{http://my-shiny.com}$
        where \index{Shiny} Server is sitting on port $80$.
...
Configuring proxies is an advanced networking topic. In most cases you'd
just put one service per server. But if you want to be able to save
money by running everything on one server, you'll need a proxy.
...
If you try to visit to your your server's public IP address or DNS in
your browser, your browser will spin for a while and nothing will
happen. That's because the AWS security group still only allows SSH
...
Complex web apps like \index{RStudio} and \index{JupyterHub} frequently
proxy traffic back to themselves. For example, when you launch a
\index{Shiny} app in RStudio, you're just opening a "headless" browser
window that gets proxied back into your session.

This works by default when those apps are on the root path $\text{/}$.
We're running RStudio and \index{JupyterHub} on subpaths, so we've got
to let the services know where they're located.

Configuring \index{RStudio Server} is already done. The
`X-RStudio-Root-Path` line in the NGINX configuration adds a header to
each request coming through the proxy that tells \index{RStudio Server}
that it's on the $\text{/rstudio}$ path.

\index{JupyterHub} needs a configuration update to let it know that it's
on a subpath. Luckily it's a very simple change. You can edit the
Jupyter configuration with
...
\index{JupyterHub} should pick up the new config when it's restarted
with
...
\index{RStudio Server} at $\text{/rstudio}$, \index{JupyterHub} at
$\text{/jupyter}$. For example, with my server at $\text{64.56.223.5}$,
I can get to \index{RStudio Server} at
...
Right now, neither the penguins model \index{API} nor the \index{Shiny} app are
available from the outside. You might want to add them to the proxy to
make them accessible. I'll leave that as an exercise for you.

::: callout-tip
It's very common to put an \index{API} and/or a \index{Shiny} app behind a
proxy. Googling "\index{Shiny} app behind nginx" or "FastAPI with nginx"
will yield good results.
:::

One thing to consider is whether the model \index{API} should be publicly
accessible at all. If the only thing calling it is the \index{Shiny}
app, maybe it shouldn't be.
