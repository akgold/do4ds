...
URL.[^3-1-networking-1] Your browser assumes defaults for the other
...
[^3-1-networking-1]: Different resources list different numbers of parts
...
[^3-1-networking-2]: Ports are also used for outbound communication.
...
In addition to the use of SSH to directly access the command line on a
remote machine, SSH can also be used to create a *tunnel* to access any
port on a remote machine. This can be useful to see an application when
you've only got SSH access, perhaps because you haven't yet configured a
stable URL for the server.

If you're not quite sure about what a port is or how

When you establish an SSH tunnel, it makes a port on the remote server
available at a port on your laptop.

An SSH tunnel to a remote can be established with
...
3.  Save a backup of the default `nginx.conf`,
    `cp /etc/nginx/nginx.conf /etc/nginx/nginx-backup.conf`.[^3-1-networking-3]
4.  Edit the Nginx configuration with `sudo vim /etc/nginx/nginx.conf`
    and replace it with:

[^3-1-networking-3]: This is generally a good practice before you start
...
upstream *default address*.[^3-1-networking-4]

[^3-1-networking-4]: There are actually a few different types of
