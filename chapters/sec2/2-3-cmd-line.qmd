...
| `cat`          | Prints a file.    |                   |
+----------------+-------------------+-------------------+
| `less`         | Prints a file,    | Can be very       |
|                | but just a        | helpful to look   |
|                | little.           | at a few rows of  |
|                |                   | csv.              |
|                |                   |                   |
|                |                   | Lazily reads      |
|                |                   | lines, so can be  |
...
|                |                   | big files.        |
+----------------+-------------------+-------------------+
| `tail`         | Look at the end   | Useful for logs,  |
|                | of a file.        | where the newest  |
|                |                   | part is *last*.   |
|                |                   |                   |
|                |                   | The `-f` flag is  |
|                |                   | useful for a live |
...
|                | beginning of a    | lines, can        |
|                | file.             | specify a         |
|                |                   | different number  |
|                |                   | with `-n <n>`.    |
+----------------+-------------------+-------------------+
| `grep`         | Search a file     | Writing regex can |
|                | using regex.      | be a pain. I      |
|                |                   | suggest testing   |
|                |                   | expressions on    |
|                |                   | regex101.com.     |
|                |                   |                   |
...
|                | o                 | used with flags.  |
|                | mpress/decompress |                   |
|                | file/directory    | Create is usually |
|                |                   |                   |
|                |                   | `t                |
|                |                   | ar -czf <archiv e |
|                |                   |  name> <file(s)>` |
...
|                |                   | v <archive name>` |
+----------------+-------------------+-------------------+
| `scp`          | Copy across `ssh` | Can use most      |
|                |                   | `ssh` flags (like |
|                |                   | `-i` and `-v`)    |
+----------------+-------------------+-------------------+
...
|                | `vim`.                |                   |
+----------------+-----------------------+-------------------+
| `:w`           | **W**rite the current | Can be combined   |
|                | file in `vim` (from   | to save and quit  |
|                | normal mode)          | in one, `:wq`     |
+----------------+-----------------------+-------------------+
| `:q`           | **Q**uit `vim` (from  | `:q!` quit        |
