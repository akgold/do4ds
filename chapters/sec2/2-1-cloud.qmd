...
Over the next few years, Amazon execs realized that like this service
was valuable to other teams at Amazon, other organizations would also be
interested in renting server capacity from AWS rather than running it
themselves. They launched Amazon Web Services (AWS) in 2006. It wasn't
the first cloud service, but it was the first one to get major
commercial traction.
:::
...
unpredictable workloads, or for temporary workloads.[^2-1-cloud-3]
...
versions of R and Python on the system, and making sure there's RStudio
and Jupyter Notebooks present.
...
models. AWS has SageMaker, Azure has Azure Machine Learning, and
Google's has Vertex AI. And there are organizations that host popular
data science tools like Posit.cloud and Saturn Cloud that you can use.
...
for the servers and storage. Every other service is just combination of
server, storage, networking, and permissions that comes with software
preinstalled or configured to make your life easier.[^2-1-cloud-4]
...
chooses cutesy names that have no relationship to the task at hand. I'm
going to try to name the services for each of the purposes I'm talking
about -- but it's worth noting that feature sets aren't exactly parallel
...
them has a service that is "just rent me a server". AWS has EC2 (Elastic
Cloud Compute) and Azure has Azure VMs, and Google has Google Compute
Engine.
...
that permissions are set in the right way. AWS has IAM (Identity
andAccess Management), Azure has Azure Active Directory, and Google has
...
free to try if you prefer Azure of GCP, but the details on how to get
...
easily. In AWS, they're called EBS (short for *Elastic Block Store*).
While it's not strictly necessary to configure an extra EBS volume to
complete the labs in this book, make sure to add one since it's
necessary if you ever want to stand up an actual environment to do work.
