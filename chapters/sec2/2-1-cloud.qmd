...
across the company --- so, instead of each team running and hosting its
own servers, they would get them from a centralized platform.
...
provider.[^2-1-cloud-2]

There's a reason why there's a small consulting industry solely focused
on saving money on AWS bills.
:::

[^2-1-cloud-1]: https://www.visualcapitalist.com/aws-powering-the-internet-and-amazons-profits/

[^2-1-cloud-2]: https://a16z.com/2021/05/27/cost-of-cloud-paradox-market-cap-cloud-lifecycle-scale-growth-repatriation-optimization/
...
resources, you can quickly spin a new server up or rescale an existing
one. This is an incredible ability for a small organization, an
organization with unpredictable workloads, or for temporary
workloads.[^2-1-cloud-3]

[^2-1-cloud-3]: By temporary workloads, I mean for example the need for
...
Service (PaaS, pronounced pass) solution. This would give me the ability
to somehow define an environment and send it somewhere to run without
worrying about the underlying servers. For example, I might want to
build a docker container with Python and Jupyter Notebook and host it
...
Sometimes you'll hear people describe PaaS or SaaS solutions as "going
serverless."
...
If you're not familiar with specific terminology used by a cloud
provider, it can be very hard to tell what service you might need from
that cloud provider --- and they don't really help the matter. It's very
common (especially in AWS) to have many different services that fulfill
similar needs, and it can be really hard to concretely tell the
difference.
...
pre-installed or configured to make your life easier.[^2-1-cloud-4]

[^2-1-cloud-4]: There are also some wild services that do specific
    things, like let you rent satellite ground station infrastructure or
    do Internet of Things (IoT) workloads. Those services are really
    cool, but so far outside the scope of this book that I'm fine with
    talking like they don't exist.
...
chooses cutesy names that have at best a tangential relationship to the
task at hand. I'm going to try to name the services for each of the
...
services and put the full name in parentheses as these are really just
trivia.
:::

Remember, at the bottom of all cloud services are servers, and each
cloud service provider has a service that is "just rent me a server".
AWS has EC2 (Elastic Cloud Compute), Azure has Azure VMs, and Google has
...
The major blob stores are AWS S3 (Simple Storage Service), Azure Blob
...
There are a variety of things you might want to do past these basic
...
| Database                 | RDS/Redshift[^2-1-cloud-5]       | Azure Database                 | Google Cloud SQL               |
| ML Platform              | SageMaker                        | Azure ML                       | Vertex AI                      |

[^2-1-cloud-5]: There are many others. These are just the most popular
...
AWS's "rent a server" is called EC2.[^2-1-cloud-6] It is the most basic
AWS service, and it's what we're going to use to get started. Navigate
to the EC2 Dashboard in the AWS console, and launch an instance.

[^2-1-cloud-6]: It's short for *Elastic Cloud Compute*, but everyone
    just says EC2.
...
Ubuntu 22.04. If you want to use a different operating system, that's
fine, but you may need to adjust the commands in this chapter and
subsequent to match.
...
R5s and many more. Within each family, there are different sizes. The
sizes vary by the family, but generally range from things below small
*nano* to multiples of *xlarge* like *24xlarge*.

We'll get into choosing the right family and size for your team's use
case in [Chaper @sec-servers]. For now, I'd recommend you get the
largest server that is free tier eligible, which is a *t2.micro* with 1
CPU and 1 Gb of memory as of this writing.
...
But if you actually want to do any data science work, you'll probably
need a substantially larger server.

It is possible to rack up really large AWS bills, so be careful. That
said, a modestly-sized server is still pretty cheap if you're only
...
just copy/paste commands if you use the same name. I'd recommend
creating a directory for this lab, perhaps something like `do4ds-lab`
and putting your keypair there. If you're not going to do that, just
make sure you keep track of where you downloaded it. Whether you're on
Windows or Mac, download the `pem` version of the key.
...
The instance ID and public IP addresses were auto-assigned. Grab the
Public IPv4 DNS address, which starts with `ec2-` and ends with
