...
-   If you're sharing data across multiple instances, you'll need some
    sort of *NAS (Network Attached Storage)*.[^2-1-cloud-8] Each of the
    cloud vendors has NAS-as-a-service offerings including AWS's *EFS
    (Elastic File System)* and *FSx*, *Azure* *File*, and GCP's
    *Filestore*.
...
[^2-1-cloud-8]: If your NAS is exclusively for Linux, you it would use
    *NFS (Network File System)* and if Windows is involved, you'd use
    *SMB (Server Message Block)*. There's also an outdated Windows NAS
    called *CIFS (Common Internet File System)* that you might see in
    on-prem systems. *Samba* is an SMB implementation for Linux you
    might see if you're connecting a Windows NAS to a Linux host.
