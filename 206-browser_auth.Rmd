...
![](images/206-auth/sso.jpeg){width="479"}
...
ticket.[^browser_auth-1] Then, when they go to the next service, they
...
[^browser_auth-1]: Depending on the system, this token or ticket can
...
the thing they are trying to do?*[^browser_auth-2]

[^browser_auth-2]: As we'll see in a second, permissions can actually
...
![](images/206-auth/acls.jpeg){width="572"}
...
groups.[^browser_auth-3]

[^browser_auth-3]: If you think about this for a minute, it may occur to
    you "isn't RBAC basically the same as doing ACLs at the group
    level?" Yes, yes it is, but it can be helpful to have groups and
    roles be separated, even if there's a 1-1 mapping.

![](images/206-auth/rbac.jpeg){width="562"}
...
![](images/206-auth/auth-pam.jpeg){width="539"}
...
![](images/206-auth/auth-ldap.jpeg){width="564"}
...
![](images/206-auth/auth-kerberos.jpeg){width="553"}
...
tokens.[^browser_auth-4]

[^browser_auth-4]: XML is a markup language, much like HTML. The good
    thing about XML is that it's very flexible. The bad thing is that
    it's relatively hard for a human to easily read.
...
![](images/206-auth/auth-saml.jpeg){width="551"}
...
![](images/206-auth/auth-oauth.jpeg)
